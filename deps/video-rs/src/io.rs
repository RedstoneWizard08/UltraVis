...
impl Reader {
    ...
    pub fn read(&mut self, stream_index: usize) -> Result<Packet> {
        ...
        loop {
            match self.input.get_next_packet().ok() {
                Some((stream, packet)) => {
    ...
    pub fn seek(&mut self, timestamp_milliseconds: i64) -> Result<()> {
        ...
        let min_ts = timestamp - LEEWAY;
        let max_ts = timestamp + LEEWAY;

        let streams = self
            .input
            .streams()
            .map(|it| it.index() as i32)
            .collect::<Vec<_>>();

        for stream in streams {
            self.input
                .seek(stream, min_ts, max_ts, timestamp, 0)
    ...
    pub fn seek_to_start(&mut self) -> Result<()> {
        ...
        for stream in streams {
            self.input
                .seek(stream, i64::MIN, i64::MAX, i64::MIN, 0)
                .map_err(Error::BackendError)?;
        }
